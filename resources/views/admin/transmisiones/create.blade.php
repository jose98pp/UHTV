...
@section('title', 'Nueva Transmisión o Video - UHTV Admin')
...
<div class="container-fluid">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <h1 class="h3 mb-0 text-gray-800 fw-bold d-flex align-items-center">
                <i class="fas fa-plus-circle text-danger me-2"></i> Registrar Transmisión, Podcast o Clip
            </h1>
        </div>
    ...
    <form action="{{ route('admin.transmisiones.store') }}" method="POST" id="transmisionForm">
        @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="tipo" id="tipoSelect" class="form-select @error('tipo') is-invalid @enderror" required>
                                    <option value="en_vivo" {{ old('tipo') == 'en_vivo' ? 'selected' : '' }}>🔴 Transmisión En Vivo</option>
                                    <option value="podcast" {{ old('tipo') == 'podcast' ? 'selected' : '' }}>🎙️ Podcast / Programa Radial</option>
                                    <option value="clip" {{ old('tipo') == 'clip' ? 'selected' : '' }}>⚡ Clip / Resumen Corto</option>
                                    <option value="programa" {{ old('tipo') == 'programa' ? 'selected' : '' }}>📺 Programa Completo Grabado</option>
                                </select>
                                @error('tipo')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="plataforma" id="plataformaSelect" class="form-select @error('plataforma') is-invalid @enderror" required>
                                    <option value="youtube" {{ old('plataforma', 'youtube') == 'youtube' ? 'selected' : '' }}>YouTube</option>
                                    <option value="facebook" {{ old('plataforma') == 'facebook' ? 'selected' : '' }}>Facebook Live / Video</option>
                                    <option value="tiktok" {{ old('plataforma') == 'tiktok' ? 'selected' : '' }}>TikTok</option>
                                    <option value="twitch" {{ old('plataforma') == 'twitch' ? 'selected' : '' }}>Twitch</option>
                                    <option value="otro" {{ old('plataforma') == 'otro' ? 'selected' : '' }}>Otro / Iframe Directo</option>
                                </select>
                                @error('plataforma')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="descripcion" 
                                      ...
                                      placeholder="Breve reseña del contenido o participantes...">{{ old('descripcion') }}</textarea>
                            @error('descripcion')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" 
                                       ...
                                       id="fechaInput" 
                                       class="form-control" 
                                       value="{{ old('fecha_transmision', now()->format('Y-m-d\TH:i')) }}">
                            </div>
                        </div>
                ...
                <div class="card shadow-sm border-0 rounded-3 mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <div class="form-check form-switch p-2 border rounded-3">
                                    <input class="form-check-input ms-0 me-2" type="checkbox" role="switch" name="activo" value="1" id="activoSwitch" {{ old('activo', 1) ? 'checked' : '' }}>
            ...
            <div class="col-lg-5">
                <div class="card shadow-sm border-0 rounded-3 sticky-top" style="top: 20px; z-index: 10;">
                    <div class="card-header bg-dark text-white py-3 d-flex justify-content-between align-items-center">
                        ...
                        <span id="previewStatusBadge" class="badge bg-secondary">Sin video cargado</span>
                    </div>
                    <div class="card-body p-0 bg-black">
                        <div class="ratio ratio-16x9 position-relative" id="previewContainer">
                            <div id="previewPlaceholder" class="w-100 h-100 d-flex flex-column align-items-center justify-content-center text-white-50 p-4 text-center">
                                <i class="fas fa-video fa-3x mb-2 opacity-50"></i>
                                <p class="mb-0 small">Pega un enlace de YouTube, Facebook, TikTok o Twitch para previsualizar aquí el reproductor</p>
                            </div>
                            <iframe id="livePreviewIframe" class="d-none w-100 h-100" src="" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                        </div>
                    </div>
                    <div class="card-body border-top bg-light">
                        <div id="previewInfo">
                            <h6 class="fw-bold text-dark mb-1" id="previewTitleDisplay">Título de la transmisión</h6>
                            <p class="small text-muted mb-2" id="previewTypeDisplay">Selecciona tipo y pega URL</p>
                            <div class="d-flex align-items-center gap-2 small">
                                <span class="badge bg-danger" id="previewLiveBadge" style="display: none;">🔴 EN VIVO</span>
                                <span class="text-muted" id="previewPlatformDisplay"><i class="fab fa-youtube text-danger"></i> YouTube</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    function updatePreview() {
        ...
        .then(data => {
            if (data.success && data.embed_url) {
                ...
                // Si no hay miniatura personalizada y viene una generada
